...
void
input_raw_string (char *buffer, size_t buffer_size)
{
...
size_t
input_string (char *buffer, size_t max_size)
{
  input_raw_string (buffer, max_size);
  size_t input_len = strnlen (buffer, max_size);
  input_len = delete_trailing_whitespace (buffer, input_len);
...
input_long_int (void)
{
  ...
  input_raw_string (input_buffer, sizeof (input_buffer));
