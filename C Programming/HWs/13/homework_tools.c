...
FILE *
checked_fopen (char *filepath, char *file_mode)
{
  errno = 0;
  FILE *file = fopen (filepath, file_mode);
  ...
  if (!file)
    {
      if (errno)
        ...
        {
          fprintf (stderr, "error: Cannot access %s: File mode: \"%s\"\n",
                   filepath, file_mode);
        }
...
size_t
checked_fread (char *buffer, size_t size, FILE *file)
{
  errno = 0;
...
size_t
checked_fwrite (char *buffer, size_t size, FILE *file)
{
  errno = 0;
...
char *
get_line (FILE *file, size_t *length)
{
  ...
  while (1)
    {
      ...
      errno = 0;
      if (!(fgets (buffer + buffer_offset, read_size + 1, file)))
        {
          if (ferror (file))
            {
              if (errno)
...
{
  size_t len;
  char *buffer = get_line (stdin, &len);
...
long
parse_long_int (char *string)
{
  char *string_end;
  errno = 0;
  long result = strtol (string, &string_end, 10);

  if (errno || string_end == string || has_text_characters (string_end))
    {
...
{
  long long_num = parse_long_int(string);

  if (long_num > INT_MAX || long_num < INT_MIN)
    {
      fprintf (stderr, "error: Integer is out of range\n");
      exit (1);
    }
