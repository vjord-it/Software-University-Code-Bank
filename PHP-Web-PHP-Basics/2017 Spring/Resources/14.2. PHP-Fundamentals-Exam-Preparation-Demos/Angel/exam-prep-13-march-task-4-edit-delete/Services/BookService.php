...
class BookService implements BookServiceInterface
{
    ...
    public function editBook($id, $author, $title, $genreId, $language, $comment = null)
    {
        // TODO: Implement editBook() method.
        $sql = 'UPDATE books SET author = ?, title= ?, genre_id =?, language = ?, comment = ? WHERE id = ?';
    ...
    public function getById($id) {
        $sql = 'SELECT id, ISBN AS isbn, title, genre_id AS genre, author, released_on AS releasedOn, comment, language FROM books WHERE id = ?';
        $stmt = $this->db->prepare($sql);
        $stmt->execute([$id]);

        $book = $stmt->fetchObject(EditBookViewData::class);
        return $book;
    }
}